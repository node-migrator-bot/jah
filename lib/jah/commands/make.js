...
Compiler.prototype.loadConfig = function (configFile) {
    ...
    if (!this.isJah) {
        this.addToBuildQueue(JAH_ROOT, '/__builtin__', 'jah.js')
    }

    // TODO Add each lib to build queue
    // TODO Need to check installed node modules for the lib
}
...
Compiler.prototype.jahFooter = function () {
    return TEMPLATES.jahFooter.substitute({ modulePaths:    "['/__builtin__', '/__builtin__/libs', '/libs', '/']"
                                          , remoteLibPaths: '[]'
                                          , globalsPaths:   '[]'
                                          })
}
    ...
    for (codePath in this.buildQueue) {
        if (this.buildQueue.hasOwnProperty(codePath)) {
            ...
            if (scriptname.indexOf(pkg.mount) === 0) {
                ...
                config = this.readConfig(path.join(codePath, 'jah.json'))
