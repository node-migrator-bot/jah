...
exports.run = function () {
    ...
    http.createServer(function (req, res) {
        ...
        if (uri.pathname == '/' + output || uri.pathname == output.replace(/^\/?public/, '')) {
            ...
            // Else if the file is a lib script, build and serve it -- /resources/libs/<libname>.js
            else if (uri.pathname.indexOf(path.join('/', compiler.config.resource_url, 'libs')) === 0) {
                var libName = path.basename(uri.pathname).replace(/\.[a-zA-Z]*$/, '');
                logger.info("Serving library", libName);

                var code = compiler.makeLib(libName);
                res.writeHead(200, {'Content-Type': 'text/javascript'});
                res.end(code);
            }

            // Else if a resource path
            else if (uri.pathname.indexOf(path.join('/', compiler.config.resource_url)) === 0) {
                resourceFilename = uri.pathname.replace(path.join('/', compiler.config.resource_url), 'src');
